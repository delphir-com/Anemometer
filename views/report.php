	...
	<div class="span4">
		...
		<select class="span4 combobox" name="fact-category_id">
			...
			<?php foreach ($category_list as $ct_row ) { ?>
				<option value="<?php echo $ct_row["category_id"] ?>" <?php if ( trim(get_var('fact-category_id')) === trim($ct_row["category_id"]) ) { echo ' SELECTED '; } ?>><?php echo $ct_row["category_name"] ?></option>
			<?php } ?>
		</select><br>
